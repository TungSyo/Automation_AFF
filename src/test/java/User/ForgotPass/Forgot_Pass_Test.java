...
public class Forgot_Pass_Test extends Base_Test {
    ...
    private Forgot_Pass_Action forgotPassAction;
    ...
    public Object[][] getForgotPassData() throws IOException, InvalidFormatException {
        ...
        for (int i = 1; i < rowCount; i++) {
            data[i - 1][0] = new Forgot_Pass_Data(
                    excel.getCellData(i, "Email"),
                    excel.getCellData(i, "NewPass"),
                    excel.getCellData(i, "ComfirmPass"),
                    ...
                    excel.getCellData(i, "Pop3"));
        }
        return data;
    }

    @Test(dataProvider = "forgotpassData", groups = { "Success", "Fail" })
    public void testForgotPass(Forgot_Pass_Data data) {
        baseAction = new Base_Action(Driver_Manager.getDriver());
        forgotPassAction = new Forgot_Pass_Action(Driver_Manager.getDriver());

        try {
            String category = data.getTestType().equalsIgnoreCase("Fail") ? "ForgotPass_Data_Fail"
                    : "ForgotPass_Data_Pass";
            Extend_Report.startTest("Forgot Pass Test - " + data.getDescription(), category);
    ...
    private void executeTestSteps(Excel_Util excelSteps, Forgot_Pass_Data data) throws Exception {
    ...
    private void handleTestException(Exception e, Forgot_Pass_Data data) {
        try {
            String screenshotPath = ScreenShotUtil.captureScreenshot(
                    Driver_Manager.getDriver(),
                    "testForgotPass_Exception",
                    "ForgotPassTest");
